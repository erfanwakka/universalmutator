...
import mutator as mutator
import solidity_handler as solidity_handler
...
def main():
    ...
    handlers = {"solidity": solidity_handler}
    ...
    mutants = mutator.mutants(source, rules, mutateTestCode=mutateTestCode, mutateBoth=mutateBoth,
                              ignorePatterns=ignorePatterns, ignoreStringOnly=not mutateInStrings)
    ...
    for mutant in mutants:
        ...
        if compileFile is None:
            ...
            mutantResult = handler(tmpMutantName, mutant, sourceFile, uniqueMutants, compileFile=compileFile)
	if(mutantResult == "INVALID"):
	    print("PROCESSING MUTANT:",str(mutant[0]) + ":", source[mutant[0] - 1][:-1], " ==> ", mutant[1][:-1], end="...")	
